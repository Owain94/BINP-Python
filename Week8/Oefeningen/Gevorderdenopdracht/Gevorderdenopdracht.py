...
def main() -> None:
    input("Neem een woord in gedachten uit het bestand \"woorden.lst\" en druk op [Enter]")
    ...
    words = open_file("woordenlijst.txt").split()
    ...
    while True:
        ...
        if inp.isalpha() and len(inp) == 1:
            write_to_file(get_word(words, inp), "woord.txt")
